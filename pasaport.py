...
def submit_form():
    ...
    vize_turu = vize_turu_var.get()

    if not ad or not soyad or not tc or not dogum_tarihi or not vize_turu:
        ...
        try:
            ...
            insert_query = "INSERT INTO pasaport(ad, soyad, tc, dogum_tarihi, vize_turu) VALUES (%s, %s, %s, %s, %s)"
            user_data = (ad, soyad, tc, dogum_tarihi, vize_turu)
    ...
    print("Vize Türü:", vize_turu)

# Ana pencereyi oluştur
root = tk.Tk()
root.title("Pasaport Sayfası")
...
tc_label = ttk.Label(root, text="TC Kimlik No:")
tc_label.grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
tc_entry = ttk.Entry(root)
tc_entry.grid(row=2, column=1, padx=10, pady=5)

dogum_tarihi_label = ttk.Label(root, text="Doğum Tarihi:")
dogum_tarihi_label.grid(row=3, column=0, padx=10, pady=5, sticky=tk.W)
dogum_tarihi_entry = ttk.Entry(root)
dogum_tarihi_entry.grid(row=3, column=1, padx=10, pady=5)

vize_turu_label = ttk.Label(root, text="Vize Türü:")
vize_turu_label.grid(row=4, column=0, padx=10, pady=5, sticky=tk.W)
vize_turu_var = tk.StringVar()
vize_turu_combobox = ttk.Combobox(root, textvariable=vize_turu_var, values=["Turistik", "İş", "Öğrenci"])
vize_turu_combobox.grid(row=4, column=1, padx=10, pady=5)
